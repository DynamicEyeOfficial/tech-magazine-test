...
if (reader?.token) {
  ...
  await expectJson("/api/notifications", (json) => json.ok && Array.isArray(json.notifications), { headers: { authorization: `Bearer ${reader.token}` } });
  await expectJson("/api/notifications/preferences", (json) => json.ok && json.preferences && typeof json.preferences.breaking === "boolean", { headers: { authorization: `Bearer ${reader.token}` } });
